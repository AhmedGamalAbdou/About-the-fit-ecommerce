...
export const createorders = createAsyncThunk('orders/createorders', async (order, thunkAPI) => {
	try {
		let config = {
			headers: {
		...
		console.log(error);
		return thunkAPI.rejectWithValue(error.response ? error.response.data.order : error.message);
	}
});
